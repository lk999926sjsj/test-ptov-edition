...
javascript: (function() {
    ...
    if (document.getElementById(HCK_BOOKMARKLET_ID)) { console.warn("HCK Shortcuts: JÃ¡ em execuÃ§Ã£o."); return; }
    ...
    const STATE = { logMessages: [], lastAnswer: null, isCycleRunning: false, ui: {} };
    ...
    function extractQuestionTextFromPage() {
        ...
        if (enunciado.trim().length < 5 && alternativasTextos.every(a => a.length < 10)) { return "Falha ao extrair a questÃ£o. ConteÃºdo insuficiente."; }
    ...
    async function doFullCycle() {
        ...
        STATE.ui.helpers.showResponse({ answer: "Processando QuestÃ£o...", detail: `Consultando especialistas...`, type: 'info' });
        try {
            ...
            let sourceIcon = 'ðŸ’¾';
            if (apiResult.source === 'live_ai') sourceIcon = 'âš¡ï¸';
            ...
            if (formattedAnswer) {
                ...
                throw new Error("IA nÃ£o retornou A-E.");
            }
        } catch (error) {
    ...
    function showLastAnswer() { if(STATE.isCycleRunning){ STATE.ui.helpers.showResponse({answer:"Aguarde", detail:"Processamento em curso...", type:'warn'}); return; } if(STATE.lastAnswer){ applyPulseToAlternative(STATE.lastAnswer); STATE.ui.helpers.showResponse({answer:`Marcando: ${STATE.lastAnswer}`, detail: "Alternativa destacada.", type:'success'}); STATE.lastAnswer = null; } else { STATE.ui.helpers.showResponse({answer:"Nada a mostrar", detail:"Execute o ciclo (2) primeiro.", type:'warn'}); } }
    
    function killSwitch() { document.removeEventListener('keydown', handleShortcuts, true); document.getElementById(HCK_BOOKMARKLET_ID)?.remove(); document.getElementById('hck-pulse-styles')?.remove(); document.getElementById('hck-notifications-container')?.remove(); }
    ...
    function setupUI() {
        ...
        menuPanel.append(titleBar, inputArea, shortcutsHelp); mainContainer.appendChild(menuPanel); document.body.appendChild(mainContainer);
    ...
    function init() {
        ...
        try {
            ...
            STATE.ui.helpers.showResponse({answer:"HCK Initialized", detail:`v${SCRIPT_VERSION}`, type:'success'});
        } catch(error) {
